...
float startTime = glfwGetTime(); // Armazena o tempo inicial
float currentTime;               // Tempo atual
float elapsedTime; 
...
struct Sprite
{
	...
	void setupSprite(int texID, vec3 position, vec3 dimensions, int nFrames, int nAnimations);
	    glm::vec2 getPMin() const {
        return position; 
    }

    glm::vec2 getPMax() const {
        return glm::vec2(position.x + dimensions.x, position.y + dimensions.y); // Calcula o ponto máximo
    }
};
...
bool checkCollision(Sprite &one, Sprite &two)
{
    ...
    bool collisionX = one.getPMin().x < two.getPMax().x &&
                      one.getPMax().x > two.getPMin().x;

    // Verificar colisão no eixo y
    bool collisionY = one.getPMin().y < two.getPMax().y &&
                      one.getPMax().y > two.getPMin().y;

    // Colisão ocorre se houver sobreposição em ambos os eixos
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Mate o amiguinho", nullptr, nullptr);
	...
	Sprite background, character, rogue;
	// Carregando uma textura (recebendo seu ID)

	// Inicializando a sprite do background
	int imgWidth, imgHeight;
	int texID = loadTexture("../Texturas/backgrounds/PNG/Battleground3/Bright/3_game_background.png", imgWidth, imgHeight);
	...
	texID = loadTexture("../Texturas/characters/PNG/enimi/walk1.png", imgWidth, imgHeight);
	character.setupSprite(texID, vec3(650.0, 200.0, 0.0), vec3(imgWidth / 6.0 * 2.0, imgHeight * 2.0, 1.0), 6, 1);

	texID = loadTexture("../Texturas/characters/PNG/Knight/walk.png", imgWidth, imgHeight);
	rogue.setupSprite(texID, vec3(100.0, 200.0, 0.0), vec3(imgWidth / 6.0 * 2.0, imgHeight * 2.0, 1.0), 6, 1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		currentTime = glfwGetTime();            // Obtém o tempo atual
		elapsedTime = currentTime - startTime;  // Calcula o tempo decorrido desde o início
		...
		if (keys[GLFW_KEY_LEFT] )
			rogue.position.x -= vel;
		if ( keys[GLFW_KEY_RIGHT] )
			rogue.position.x += vel;
		if ( keys[GLFW_KEY_UP] )
			rogue.position.y += vel;
		...
		now = glfwGetTime();
		float dt2 = now - rogue.lastTime;
		if (dt2 >= 1.0 / rogue.FPS)
		{
			rogue.iFrame = (rogue.iFrame + 1) % rogue.nFrames; // incrementando ciclicamente o indice do Frame
			rogue.lastTime = now;
		}	
		offsetTex.s = rogue.iFrame * rogue.d.s;
		offsetTex.t = 0.0;
		glUniform2f(glGetUniformLocation(shaderID, "offsetTex"), offsetTex.s, offsetTex.t);
		drawSprite(rogue, shaderID);

		        if (checkCollision(character, rogue))
        {
			//glfwTerminate();
            std::cout << "O amigo foi morto com sucesso!" << std::endl;
